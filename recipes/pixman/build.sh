...
WORK="${ALTITUDE_RECIPE_WORK:-$ROOT/out/source-work/pixman}"
VERSION=0.46.4
...
TARBALL="$(bash "$ROOT/scripts/source-fetch.sh" pixman)"
...
for tool in "$CC" "$AR" "$STRIP" "$PKG_CONFIG"; do
  [ -x "$tool" ] || {
    echo "pixman: missing build tool: $tool" >&2
    exit 1
  }
done
for tool in meson ninja; do
  command -v "$tool" >/dev/null ||
    { echo "pixman: missing host build tool: $tool" >&2; exit 1; }
done
...
"$STRIP" --strip-unneeded "$PAYLOAD"/usr/lib/libpixman-1.so.* 2>/dev/null || true
...
} > "$PAYLOAD/usr/share/altitude/sources/pixman.build"

bash "$ROOT/rootfs/bin/altpkg-build" \
  "$ROOT/recipes/pixman/MANIFEST" "$PAYLOAD" \
  "$OUT/altitude-pixman-$VERSION-amd64.altpkg"
