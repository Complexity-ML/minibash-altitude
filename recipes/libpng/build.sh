...
WORK="${ALTITUDE_RECIPE_WORK:-$ROOT/out/source-work/libpng}"
...
TARBALL="$(bash "$ROOT/scripts/source-fetch.sh" libpng)"
...
for tool in gcc ar ranlib strip; do
  [ -x "$TOOLCHAIN/bin/$TARGET-$tool" ] || {
    echo "libpng: missing Altitude tool: $TOOLCHAIN/bin/$TARGET-$tool" >&2
    exit 1
  }
done
...
"$TOOLCHAIN/bin/$TARGET-strip" --strip-unneeded \
  "$PAYLOAD"/usr/lib/libpng16.so.* 2>/dev/null || true
...
} > "$PAYLOAD/usr/share/altitude/sources/libpng.build"

bash "$ROOT/rootfs/bin/altpkg-build" \
  "$ROOT/recipes/libpng/MANIFEST" "$PAYLOAD" \
  "$OUT/altitude-libpng-$VERSION-amd64.altpkg"
