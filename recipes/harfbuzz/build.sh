...
WORK="${ALTITUDE_RECIPE_WORK:-$ROOT/out/source-work/harfbuzz}"
VERSION=12.3.2
...
TARBALL="$(bash "$ROOT/scripts/source-fetch.sh" harfbuzz)"
...
for tool in "$CC" "$CXX" "$AR" "$STRIP" "$PKG_CONFIG"; do
  [ -x "$tool" ] || {
    echo "harfbuzz: missing build tool: $tool" >&2
    exit 1
  }
done
for tool in meson ninja; do
  command -v "$tool" >/dev/null ||
    { echo "harfbuzz: missing host build tool: $tool" >&2; exit 1; }
done
...
"$STRIP" --strip-unneeded "$PAYLOAD"/usr/lib/libharfbuzz*.so.* 2>/dev/null || true
cp -a "$PAYLOAD/usr/." "$SYSROOT/usr/"

{
  echo "Source: harfbuzz"
  echo "Version: $VERSION"
  echo "SHA256: $(sha256sum "$TARBALL" | awk '{print $1}')"
  echo "Build: Meson shared and static cross $TARGET"
  echo "Compiler: $("$CXX" --version | head -1)"
} > "$PAYLOAD/usr/share/altitude/sources/harfbuzz.build"

bash "$ROOT/rootfs/bin/altpkg-build" \
  "$ROOT/recipes/harfbuzz/MANIFEST" "$PAYLOAD" \
  "$OUT/altitude-harfbuzz-$VERSION-amd64.altpkg"
