...
WORK="${ALTITUDE_RECIPE_WORK:-$ROOT/out/source-work/libxau}"
...
TARBALL="$(bash "$ROOT/scripts/source-fetch.sh" libXau)"
...
"$TOOLCHAIN/bin/$TARGET-strip" --strip-unneeded \
  "$PAYLOAD"/usr/lib/libXau.so.* 2>/dev/null || true
...
} > "$PAYLOAD/usr/share/altitude/sources/libxau.build"

bash "$ROOT/rootfs/bin/altpkg-build" \
  "$ROOT/recipes/libxau/MANIFEST" "$PAYLOAD" \
  "$OUT/altitude-libxau-$VERSION-amd64.altpkg"
