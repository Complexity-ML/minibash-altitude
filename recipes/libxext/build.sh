...
WORK="${ALTITUDE_RECIPE_WORK:-$ROOT/out/source-work/libxext}"
...
TARBALL="$(bash "$ROOT/scripts/source-fetch.sh" libXext)"

export PATH="$FORGE/bin:$TOOLCHAIN/bin:$PATH"
export PKG_CONFIG_LIBDIR="$SYSROOT/usr/lib/pkgconfig:$SYSROOT/usr/share/pkgconfig"
export PKG_CONFIG_SYSROOT_DIR="$SYSROOT"

for dep in x11 xextproto; do
  "$FORGE/bin/pkg-config" --exists "$dep" ||
    { echo "libxext: target dependency missing from $SYSROOT: $dep" >&2; exit 1; }
done
...
"$TOOLCHAIN/bin/$TARGET-strip" --strip-unneeded \
  "$PAYLOAD"/usr/lib/libXext.so.* 2>/dev/null || true
...
} > "$PAYLOAD/usr/share/altitude/sources/libxext.build"

bash "$ROOT/rootfs/bin/altpkg-build" \
  "$ROOT/recipes/libxext/MANIFEST" "$PAYLOAD" \
  "$OUT/altitude-libxext-$VERSION-amd64.altpkg"
