...
WORK="${ALTITUDE_RECIPE_WORK:-$ROOT/out/source-work/libx11}"
...
TARBALL="$(bash "$ROOT/scripts/source-fetch.sh" libX11)"

export PATH="$FORGE/bin:$TOOLCHAIN/bin:$PATH"
export PKG_CONFIG_LIBDIR="$SYSROOT/usr/lib/pkgconfig:$SYSROOT/usr/share/pkgconfig"
export PKG_CONFIG_SYSROOT_DIR="$SYSROOT"

for dep in xproto kbproto xtrans xcb; do
  "$FORGE/bin/pkg-config" --exists "$dep" ||
    { echo "libx11: target dependency missing from $SYSROOT: $dep" >&2; exit 1; }
done
...
(
  ...
  ./configure --host="$TARGET" --prefix=/usr --libdir=/usr/lib \
    --enable-shared --enable-static --disable-specs \
...
"$TOOLCHAIN/bin/$TARGET-strip" --strip-unneeded \
  "$PAYLOAD"/usr/lib/libX11*.so.* 2>/dev/null || true
...
} > "$PAYLOAD/usr/share/altitude/sources/libx11.build"

bash "$ROOT/rootfs/bin/altpkg-build" \
  "$ROOT/recipes/libx11/MANIFEST" "$PAYLOAD" \
  "$OUT/altitude-libx11-$VERSION-amd64.altpkg"
