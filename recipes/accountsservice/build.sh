...
for dep in dbus-1 gio-2.0 gio-unix-2.0 glib-2.0 json-c libelogind polkit-gobject-1; do
...
meson setup "$WORK/build" "$WORK/source" \
  ...
  -Dadmin_group=wheel -Dminimum_uid=1000 \
  -Dlibxcrypt=false -Dintrospection=false -Dvapi=false \
