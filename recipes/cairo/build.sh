...
WORK="${ALTITUDE_RECIPE_WORK:-$ROOT/out/source-work/cairo}"
VERSION=1.18.4
...
TARBALL="$(bash "$ROOT/scripts/source-fetch.sh" cairo)"
...
for tool in "$CC" "$CXX" "$AR" "$STRIP" "$PKG_CONFIG"; do
  [ -x "$tool" ] || {
    echo "cairo: missing build tool: $tool" >&2
    exit 1
  }
done
for tool in meson ninja; do
  command -v "$tool" >/dev/null ||
    { echo "cairo: missing host build tool: $tool" >&2; exit 1; }
done
...
meson setup "$WORK/build" "$WORK/source" \
  ...
  -Dtests=disabled -Dfreetype=enabled -Dfontconfig=enabled -Dpng=enabled \
  -Dglib=enabled \
  -Dxlib=disabled -Dxcb=disabled -Dgtk2-utils=disabled \
  -Dspectre=disabled -Dlzo=disabled
meson compile -C "$WORK/build"
DESTDIR="$PAYLOAD" meson install -C "$WORK/build"

"$STRIP" --strip-unneeded "$PAYLOAD"/usr/lib/libcairo*.so.* 2>/dev/null || true
...
} > "$PAYLOAD/usr/share/altitude/sources/cairo.build"

bash "$ROOT/rootfs/bin/altpkg-build" \
  "$ROOT/recipes/cairo/MANIFEST" "$PAYLOAD" \
  "$OUT/altitude-cairo-$VERSION-amd64.altpkg"
