...
WORK="${ALTITUDE_RECIPE_WORK:-$ROOT/out/source-work/fribidi}"
VERSION=1.0.16
...
TARBALL="$(bash "$ROOT/scripts/source-fetch.sh" fribidi)"
...
for tool in "$CC" "$AR" "$STRIP" "$PKG_CONFIG"; do
  [ -x "$tool" ] || { echo "fribidi: missing build tool: $tool" >&2; exit 1; }
done
for tool in meson ninja; do
  command -v "$tool" >/dev/null ||
    { echo "fribidi: missing host build tool: $tool" >&2; exit 1; }
done
...
"$STRIP" --strip-unneeded "$PAYLOAD"/usr/lib/libfribidi.so.* 2>/dev/null || true
...
} > "$PAYLOAD/usr/share/altitude/sources/fribidi.build"

bash "$ROOT/rootfs/bin/altpkg-build" \
  "$ROOT/recipes/fribidi/MANIFEST" "$PAYLOAD" \
  "$OUT/altitude-fribidi-$VERSION-amd64.altpkg"
