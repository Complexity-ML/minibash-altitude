...
echo "registry /bad/path bool system maybe" >> "$TMP/invalid.conf"
...
cat > "$TMP/dependency.conf" <<'EOF'
bdbconf 1
dependency graphical after displayd
EOF
set +e
bash "$ROOT/rootfs/bin/bdbconf" check "$TMP/dependency.conf" >/dev/null 2>&1
status=$?
set -e
[ "$status" -ne 0 ]
